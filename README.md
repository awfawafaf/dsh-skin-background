...
- **系统文件窗口选图**:点「选择图片并保存…」弹出系统原生「打开」对话框,图片存入插件资产库(磁盘文件),设置文档只保存服务 URL —— 库再大也不拖慢切换;
- **保存库**:可随时「应用」历史图片或「删除」;
- **存储文件夹自选**:设置里用原生目录选择器指定壁纸存储位置(默认 `~/.dsh/data/dsh-skin-background`),并支持**从文件夹一键导入**现成图片库;
- **支持 GIF 动图**:动图作为背景会播放(静态图上限 12MB,动图上限 15MB);
...
2. 「背景」行点「选择图片并保存…」从系统文件窗口选图 —— 自动存入保存库并设为当前图;或点「从文件夹导入」把已选的存储文件夹里的图片一次收进保存库;
3. 「存储文件夹」行用系统目录选择器更换壁纸存放位置;
4. 「已保存」列表可「应用」/「删除」;
5. 「透明度」压暗亮图;「边栏透明度」调节玻璃通透度 —— 拖动即时预览;
6. 切回「经典」或「深海女仆」即还原原皮肤外观。
...
interface BackgroundSettings {
  ...
  assetDir: string        // 用户选择的存储文件夹;空 = 插件默认位置
  items: BackgroundItem[] // 保存库
}
interface BackgroundItem {
  id: string
  name: string            // 原文件名(仅显示)
  url: string             // 资产服务路径 /skin-background/assets/<file>
}
```

图片本体由插件 host 半的资产路由管理(`/skin-background/upload` 上传、`/skin-background/list` 列举、`/skin-background/assets/<file>` 读取/删除),设置文档因此保持轻量,写回一次只带 URL。设置读写走插件自有 `/skin-background/settings` 路由,不依赖 Web BFF 的命名空间白名单,第三方安装开箱即用。
