window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function BackgroundRow({ t, useStore, update, applyItem, previewOpacity, previewChrome }) {
		...
		*/
		...
		const zh = {
			...
			apply: "应用",
			delete: "删除",
			fileTooLarge: "图片太大(静态图限 4MB,动图限 15MB),请换一张更小的",
		...
		const en = {
			...
			apply: "Apply",
			delete: "Delete",
			fileTooLarge: "Image too large (4MB for stills, 15MB for GIFs); pick a smaller one",
		...
		*/
		function apply(ctx) {
			...
			const imageValue = (item) => item !== void 0 ? `url("${item.dataUrl}")` : document.body.hasAttribute("data-ds-dark-theme") ? DARK_FALLBACK_GRADIENT : LIGHT_FALLBACK_GRADIENT;
			...
			*/
			...
			const skin = {
				...
				apply: () => {
					active = true;
					if (host.getSnapshot().value === void 0) {} else setup();
					return teardownSkin;
				}
			};
